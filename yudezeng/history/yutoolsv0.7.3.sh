...
apui=$(whiptail --clear --title "proot 容器" --menu "建议优先选择第一项，跨架构容器可能会导致性能损失" 18 50 7  "1" "安装$ARCH_qp Linux 系统 (proot容器)" "01" "使用debootstrap来手动构建系统(可能会出现bug)" "02" "yti-arch(Arch Linux ARM)(arm64)" "03" "ubuntu 22.04 server" "2" "安装恢复包" "3" "导出系统" "4" "arm64设备安装Linux x86_64系统(proot+qemu)" "5" "arm64设备安装Linux x86系统(proot+qemu)" "10" "退出"  3>&1 1>&2 2>&3)
case $apui in
1)
     ARCH=$(ARCH_)
     AH=$(ARCH_)

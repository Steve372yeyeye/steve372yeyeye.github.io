...
readmek(){
    echo yutools v0.7.31
    echo 对debian的兼容性完善
    echo 添加了对ubuntu最新版的支持
    
    echo "yutools v0.7.4 dev"
    echo 修复了部分已知bug
    echo 增加了对chroot容器的支持
    echo "archlinux暂不支持自动配置(chroot)"
    echo 
    sleep 1s
    echo 即将加载功能...
    sleep 0.6s
...
cat > setlang <<- EOM
#/bin/bash
echo y | pacman -S wget curl git neofetch tzdata 
sleep 1
echo 正在设置语言为中文
...
FIN_(){
    if [ -e centos-amd64 ]
    then
    cpu=" -cpu Snowridge-v4 2>/dev/null"
    fi
echo "配置qemu"
sleep 2
...
AUTOFIN(){
    ...
    if [ $sys_name = kinetic ]
     ...
     echo 换软件源
     echo "
     deb http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic main restricted universe multiverse
# deb-src http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic main restricted universe multiverse
deb http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-updates main restricted universe multiverse
# deb-src http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-updates main restricted universe multiverse
deb http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-backports main restricted universe multiverse
# deb-src https://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-backports main restricted universe multiverse
deb http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-security main restricted universe multiverse
# deb-src http://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-security main restricted universe multiverse

# 预发布软件源，不建议启用
# deb https://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-proposed main restricted universe multiverse
# deb-src https://mirrors.tuna.tsinghua.edu.cn/$APKH/ kinetic-proposed main restricted universe multiverse ">sources.list
rm -rfv /etc/apt/sources.list
cp -rv ./sources.list /etc/apt/
apt update && autofin
EOM
fi
    if [ $sys_name = jammy-server ]
     then
     cat > autoset <<- EOM
     echo 换软件源
     echo "
...
deb http://mirrors.tuna.tsinghua.edu.cn/debian/ bookworm-updates main contrib non-free
deb http://mirrors.tuna.tsinghua.edu.cn/debian/ bookworm-backports main contrib non-free
deb http://mirrors.tuna.tsinghua.edu.cn/debian-security bookworm-updates main contrib non-free
">sources.list
rm -rfv /etc/apt/sources.list
cp -rv ./sources.list /etc/apt/
apt update
...
SYS_SELECT_(){
opti=$(whiptail --clear --title "选择系统(proot containers install menu)" --menu "选择你要安装的系统" 15 45 7 "1" "debian" "2" "ubuntu" "3" "kali" "4" "centos" "5" "arch" "6" "fedora"  "7" "从本地安装" "10" "退出"  3>&1 1>&2 2>&3)
case $opti in
	1)input=$(whiptail --clear --title "选择debian的哪个系统？" --menu "建议选择 debian sid" 15 45 7 "1" "debian 10 (buster)" "2" "debian 11 (bullseye)" "3" "debian sid (sid)" "4" "debian 12 (bookworm)" "0" "退出" 3>&1 1>&2 2>&3)
		name=debian
		case $input in
			1) echo "即将下载安装debian 10 (buster)"
		sys_name=buster
		DEF_CUR="https://mirrors.bfsu.edu.cn/lxc-images/images/debian/buster/$ARCH/default/" ;;
	2) echo "即将下载安装debian 11 (bullseye)"        
		sys_name=bullseye  
		DEF_CUR="https://mirrors.bfsu.edu.cn/lxc-images/images/debian/bullseye/$ARCH/default/" ;;
	3) echo "即将下载安装debian sid (sid)"
		sys_name=sid
		DEF_CUR="https://mirrors.bfsu.edu.cn/lxc-images/images/debian/sid/$ARCH/default/" ;;
	4) echo "即将下载安装debian 12 (bookworm)"
		sys_name=bookworm
		DEF_CUR="https://mirrors.bfsu.edu.cn/lxc-images/images/debian/bookworm/$ARCH/default/" ;;
		9) MAIN ;;
		0) echo -e "\nexit"
			sleep 1                           
			exit 0 ;;                           
...
2) input=$(whiptail --clear --title "选择ubuntu的哪个系统？" --menu "请选择" 15 45 7 "1" "ubuntu 18.04 (bionic)" "2" "ubuntu 20.04 (focal)" "3" "ubuntu 21.04 (impish)" "4" "ubuntu 22.04 (jammy) " "5" "ubuntu 22.10 (kinetic)" "0" "退出"  3>&1 1>&2 2>&3 )                     
	name=ubuntu
	case $input in
	1) echo "即将下载安装ubuntu(bionic)"
		sys_name=bionic
...
SYS_SELECT_chroot(){
opti=$(whiptail --clear --title "选择系统(chroot containers install menu)" --menu "选择你要安装的系统" 15 45 7 "1" "debian" "2" "ubuntu" "3" "kali" "4" "centos" "5" "arch" "6" "fedora"  "7" "从本地安装" "10" "退出"  3>&1 1>&2 2>&3)
case $opti in
	1)input=$(whiptail --clear --title "选择debian的哪个系统？" --menu "建议选择 debian sid" 15 45 7 "1" "debian 10 (buster)" "2" "debian 11 (bullseye)" "3" "debian sid (sid)" "4" "debian 12 (bookworm)" "0" "退出" 3>&1 1>&2 2>&3)
		name=debian
		case $input in
...
BAGNAME="rootfs.tar.xz"
	...
	curl -o ${BAGNAME} ${DEF_CUR}
		...
		mkdir $dir
echo -e "正在解压系统包"
		tar xvf ${BAGNAME} -C $dir 2>/dev/null
		rm rootfs.tar.xz
                echo -e "'$sys_name-$AH'系统已下载，文件夹名为$dir"
}
FIN_chroot(){
echo "写入chroot启动脚本"
sleep 1
...
remove_con(){
    df -ha >>p.log
    if ! cat p.log |grep 'c-$sys_name-$AH' ;then
    rm -rfv 'c-$sys_name-$AH'
    rm -rfv 'c-$sys_name-$AH'.sh
    echo 系统已经移除.
    exit 1
    else
su -c "umount -lvf 'c-$sys_name-$AH'/dev"
su -c "umount -lvf 'c-$sys_name-$AH'/sys"
su -c "umount -lvf 'c-$sys_name-$AH'/proc"
su -c "umount -lvf 'c-$sys_name-$AH'/media/sd"
su -c "umount -lvf 'c-$sys_name-$AH'/sdcard"
su -c "mount -vo bind /dev/pts 'c-$sys_name-$AH'/dev/pts"
if [ -e 'c-$sys_name-$AH' ]
then
chmod 777 -R 'c-$sys_name-$AH'
proot -0 rm -rfv 'c-$sys_name-$AH'
remove_con
fi
    rm -rf p.log
    remove_con
...
echo 按回车继续
read enter
put=$(whiptail --clear --title "'$sys_name-$AH' chroot系统选项" --menu "请选择一项" 15 35 4 'L' '进入容器' 'Y' '卸载容器' 'R' '删除容器' 'E' '退出启动脚本' 3>&1 1>&2 2>&3)
case ${put} in
y*|Y*)
su -c "umount -lvf 'c-$sys_name-$AH'/dev"
su -c "umount -lvf 'c-$sys_name-$AH'/sys"
...
su -c "umount -lvf 'c-$sys_name-$AH'/media/sd"
su -c "umount -lvf 'c-$sys_name-$AH'/sdcard"
su -c "umount -lvf'c-$sys_name-$AH'/dev/pts"
rm -rf point.log
exit 1
;;
n*|N*|L*|l*)
if ! cat point.log |grep 'c-$sys_name-$AH' ;then
su -c "mount -vo bind /dev 'c-$sys_name-$AH'/dev"
su -c "mount -vo bind /sys 'c-$sys_name-$AH'/sys"
su -c "mount -vo bind /proc 'c-$sys_name-$AH'/proc"
su -c "mount -vo bind /sdcard 'c-$sys_name-$AH'/media/sd"
su -c "mount -vo bind /sdcard 'c-$sys_name-$AH'/sdcard"
su -c "mount -vo bind /dev/pts 'c-$sys_name-$AH'/dev/pts"
rm -rf point.log
fi
su -c "chroot 'c-$sys_name-$AH'/ /usr/bin/env -i LANG=zh_CN.UTF-8 TERM=xterm-256color /bin/su -l root "
;;
R*|r*)
remove_con
;;
...
main_
rm -rf point.log'>>c-$sys_name-$AH.sh

echo "授予启动脚本执行权限"
sleep 1
chmod +x c-$sys_name-$AH.sh
if [ -e ${PREFIX}/etc/bash.bashrc ]; then
	if ! grep -q 'pulseaudio' ${PREFIX}/etc/bash.bashrc; then
		sed -i "1i\pkill -9 pulseaudio" ${PREFIX}/etc/bash.bashrc
	fi
...
SYS_SET_chroot(){
	...
	rm $dir/etc/hostname 2>/dev/null
	echo "$hostinfo" > $dir/etc/hostname
	echo "127.0.0.1 $hostinfo" > $dir/etc/hosts
	rm -rf $dir/etc/resolv.conf &&
	echo "nameserver 223.5.5.5
nameserver 223.6.6.6
nameserver 114.114.114.114" >$dir/etc/resolv.conf
echo "设置时区"
sleep 1
     rm systeminfo.log
	echo "export  TZ='Asia/Shanghai'" >> $dir/root/.bashrc
	echo "export  TZ='Asia/Shanghai'" >> $dir/etc/profile
	echo "export PULSE_SERVER=tcp:127.0.0.1:4173" >> $die/etc/profile
	echo "export PULSE_SERVER=tcp:127.0.0.1:4173" >> $dir/root/bashrc
	if grep -q 'ubuntu' "$dir/etc/os-release" ; then
        touch "$dir/root/.hushlogin"
fi
mkdir -v $dir/media/sd
mkdir -v $dir/sdcard
}
AUTOFIN_chroot(){
    APKH=ubuntu-ports
...
EOM
    chmod 777 autofin
    chmod 777 autoset
    cp -rv autoset c-$sys_name-$AH/usr/bin/autoset
    cp -rv autofin c-$sys_name-$AH/usr/bin/autofin
    rm autoset autofin
su -c "mount -vo bind /dev 'c-$sys_name-$AH'/dev"
su -c "mount -vo bind /sys 'c-$sys_name-$AH'/sys"
su -c "mount -vo bind /proc 'c-$sys_name-$AH'/proc"
su -c "mount -vo bind /sdcard 'c-$sys_name-$AH'/media/sd"
su -c "mount -vo bind /sdcard 'c-$sys_name-$AH'/sdcard"
su -c "chroot c-$sys_name-$AH/ /usr/bin/env -i HOME=/root LANG=zh_CN.UTF-8 TERM=xterm-256color /bin/autoset"
su -c "umount -lvf 'c-$sys_name-$AH'/dev"
su -c "umount -lvf 'c-$sys_name-$AH'/sys"
su -c "umount -lvf 'c-$sys_name-$AH'/proc"
su -c "umount -lvf 'c-$sys_name-$AH'/media/sd"
...
apui=$(whiptail --clear --title "proot 容器" --menu "建议优先选择第一项，跨架构容器可能会导致性能损失" 18 50 7  "1" "安装$ARCH_qp Linux 系统 (proot容器)" "01" "使用debootstrap来手动构建系统(可能会出现bug)" "02" "ubuntu 22.04 server" "2" "安装恢复包" "3" "导出系统" "4" "arm64设备安装Linux x86_64系统(proot+qemu)" "5" "arm64设备安装Linux x86系统(proot+qemu)" "10" "退出"  3>&1 1>&2 2>&3)
case $apui in
1)
     ARCH=$(ARCH_)
     AH=$(ARCH_)
...
3)echo "请输入此目录的容器系统目录名称"
      ...
      BAGNAME="rootfs.tar.xz"
		...
		aria2c -o ${BAGNAME} -x 16 -s 16 https://down.xb6868.com/xuebi1/proot/ubuntu%20dev%2022.10/root.tar.xz
		if [ $? -ne 0 ]; then
			echo -e "${RED}下载失败${RES}\n"
			echo 按回车继续
			read enter
...
;;
      ...
      BAGNAME="rootfs.tar.xz"
		...
		aria2c -o ${BAGNAME} -x 16 -s 16 https://mirrors.tuna.tsinghua.edu.cn/ubuntu-cloud-images/server/jammy/current/jammy-server-cloudimg-arm64-root.tar.xz
		if [ $? -ne 0 ]; then
			echo -e "${RED}下载失败${RES}\n"
			echo 按回车继续
			read enter
...
chroot_containers_manager(){
    ...
    *)echo 'root [ failed ✘ ]'
    ...
    sleep 0.5
    apui=$(whiptail --clear --title "chroot 容器" --menu "" 18 50 7  "1" "安装$ARCH_qp Linux 系统 (chroot容器)" "01" "使用debootstrap来手动构建系统(可能会出现bug)"  "2" "安装恢复包" "3" "导出系统" "10" "退出"  3>&1 1>&2 2>&3)
case $apui in
1)
     ARCH=$(ARCH_)
     AH=$(ARCH_)
     ...
     debian|ubuntu)
     AUTOFIN_chroot
     echo -e "现在可以执行 ./c-$sys_name-$AH.sh 运行 $sys_name-$AH系统"
     exit 1
     ;;
     *)
     echo -e "现在可以执行 ./c-$sys_name-$AH.sh 运行 $sys_name-$AH系统"
...
ldp
sleep 0.5
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 18 50 10 "0" "Android-Termux工具(内置修复termux openssl)" "1" "proot 容器(running linux in termux)" "2" "chroot 容器" "3" "返回到旧版本页面" "4" "xbtools(雪碧提供支持)" "5" "检查更新" "6" "下载脚本到本地" "10" "退出"  3>&1 1>&2 2>&3)
LODU
}
####
setup(){
...
;;
2)
itu=$(whiptail --clear --title "安装桌面环境" --menu "请选择需要安装的桌面环境，建议选择xfce4" 15 45 7 "0" "xfce4(兼容性很强,性能不需要太多)" "1" "kde(桌面环境非常的优美)" "3" "gnome(推荐ubuntu 22.04)" "10" "退出"  3>&1 1>&2 2>&3)
case $itu in
0)
...
;;
3)
apt install gnome dbus-x11 lxtask -y
apt install -y language-pack-zh-han* language-pack-gnome-zh-han* 
apt install -y fonts-noto-cjk*
setvnc
mkdir .vnc
...
#------------------
set_session_env() {
    SESSION_1="gnome-shell-x11"
    SESSION_2="gnome-flashback-metacity"
...
open_terminal() {
    for i in x-terminal-emulator xfce4-terminal konsole qterminal lxterminal gnome-terminal lxterm xterm mate-ter>
...
start_session() {
    for i in ${SESSION_1} ${SESSION_2}; do
        if [[ -n $(command -v $i) ]]; then
            exec ${DBUS_CMD} ${i} ${@}                                                                                        break
        fi
    done
}
set_session_env
open_terminal
start_session ${@}'>${HOME}/.vnc/xstartup
;;
*)MENU
;;
10)
...
;;
4)
sudo apt install -y proot zstd pulseaudio debootstrap
main_
;;
5)
dti
...
system_tools(){
    input=$(whiptail --clear --title "install app" --menu "请选择需要安装的app" 15 45 7 "0" "htop" "1" "lxtask" "2" "neofetch" "3" "gnome-disks" "10" "退出"  3>&1 1>&2 2>&3)
case $input in
0)
apt install -y htop
MENU
...
#脚本功能
debootstrap_(){
      if [ ! $(command -v debootstrap) ]
      then 
      apt install -y debootstrap
      fi
    input=$(whiptail --clear --title "选择系统" --menu "此功能为手动构建系统(建议有linux基础的人使用)" 15 45 8 "1" "debian 10 (buster)" "2" "debian 11 (bullseye)" "3" "debian sid (sid)" "4" "debian 12 (bookworm)" "5" "ubuntu 18.04 (bionic)" "6" "ubuntu 20.04 (focal)" "7" "ubuntu 21.04 (impish)" "8" "ubuntu 22.04 (jammy)" "0" "退出" 3>&1 1>&2 2>&3)
		name=ubuntu
		case $input in
			1) echo "即将下载安装debian 10 (buster)"
		sys_name=buster
		DEF_CUR="https://mirrors.bfsu.edu.cn/debian/" ;;
	2) echo "即将下载安装debian 11 (bullseye)"        
		sys_name=bullseye  
		DEF_CUR="https://mirrors.bfsu.edu.cn/debian/" ;;
	3) echo "即将下载安装debian sid (sid)"
	...
	4) echo "即将下载安装debian 12 (bookworm)"
		sys_name=bookworm
		DEF_CUR="https://mirrors.bfsu.edu.cn/debian/" ;;
	5) echo "即将下载安装ubuntu(bionic)"
		sys_name=bionic
		DEF_CUR="https://mirrors.bfsu.edu.cn/ubuntu-ports" ;;
	6) echo "即将下载安装ubuntu(focal)"
		sys_name=focal
		DEF_CUR="https://mirrors.bfsu.edu.cn/ubuntu-ports/" ;;
	7) echo "即将下载安装ubuntu(impish)"
		sys_name=impish
		DEF_CUR="https://mirrors.bfsu.edu.cn/ubuntu-ports/" ;;
	8) echo "即将下载安装ubuntu(jammy)"
	sys_name=jammy
	DEF_CUR="https://mirrors.bfsu.edu.cn/ubuntu-ports/" ;;
	9) MAIN ;;                         
		0) echo -e "\nexit"     
...
debootstrap_d(){
    debootstrap --arch=$ARCH $sys_name $sys_name-$AH ${DEF_CUR}
    cat > dfin <<-eof
    dpkg -i /var/cache/apt/archives/*.deb
eof
chmod -v 777 dfin
cp -v dfin $sys_name-$AH/usr/local/bin/dfin
    unset LD_PRELOAD
proot --bind=/vendor --bind=/system --bind=/data/data/com.termux/files/usr --bind=/storage --bind=/storage/self/primary:/sdcard --bind=/data/data/com.termux/files/home --bind=/data/data/com.termux/cache --bind=/data/dalvik-cache --bind=$sys_name-$AH/tmp:/dev/shm --bind=/sys --bind=/proc/self/fd/2:/dev/stderr --bind=/proc/self/fd/1:/dev/stdout --bind=/proc/self/fd/0:/dev/stdin --bind=/proc/self/fd:/dev/fd --bind=/proc --bind=/dev/urandom:/dev/random --bind=/dev --root-id --cwd=/root -L --kernel-release=5.17.18-perf --sysvipc --link2symlink --kill-on-exit --rootfs=$sys_name-$AH/ /usr/bin/env -i HOME=/root LANG=zh_CN.UTF-8 TERM=xterm-256color PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin /usr/bin/bash /usr/local/bin/dfin
    SYS_SET
    FIN
}
setdebian(){
    ...
    case $sys_name in
    bullseye)
    down_url="https://down.xb6868.com/xuebi1/proot/debs-bullseye.tar"
    ;;
    buster)
    down_url="https://down.xb6868.com/xuebi1/proot/debs-buster.tar"
    ;;
    sid)
    down_url="https://down.xb6868.com/xuebi1/proot/debs-sid.tar"
    ;;
    *) echo 
    ;;
    esac
    echo 获取下载链接...
    sleep 0.5
    echo "开始下载(Using aria2)"
    sleep 0.5
    aria2c -s 2 -x 4 -o debs.tar $down_url
    echo 开始解压缩文件...
    mkdir tmp
    tar xf debs.tar -C tmp
    rm debs.tar
    sleep 1
    echo 移动文件...
    mv ./tmp $sys_name-$AH/tmp_deb
    echo 准备配置...
     cat > dfin <<-eof
    dpkg -i /tmp_deb/*.deb
    rm -rf /tmp_deb
...
setvnc(){
    apt update
    apt install tigervnc* -y 2>/dev/null
    echo 'tigervncserver :2
    echo Your vnc aadress is <ip address>:2 
    echo type vnc-stop stop your vnc server'>>/usr/local/bin/vnc-start
    echo 'vncserver -kill :2
    stopped your vnc server' >>/usr/local/bin/vnc-stop
    chmod -v 777 /usr/local/bin/vnc-*
    echo you can type vnc-start starting vnc server
}
intp(){
    date_t=`date +"%D"`
    hiopk="$date_t+1"
if ! grep -q $hiopk ".date_tmp.log" 2>/dev/null; then
      echo $hiopk >>.date_tmp.log 2>&1
      if [ $PATHG = GNU/Linux ]
      then
      echo 检查执行此功能所需要的依赖
      apt update
...
change_the_password() {
    ...
    echo 输入su -l $USER_NAME 进入刚刚创建的用户
...
if ! grep -q $date_t ".date_tmp.log" 2>/dev/null; then
        echo 检查软件更新
pacman -Syy
echo y |pacman -S git neofetch curl wget libnewt aria2
        echo $date_t >>.date_tmp.log 2>&1
fi
date_t="# zh_CN.UTF-8 UTF-8 "
if ! grep -q $date_t "/etc/locale.gen" 2>/dev/null; then
        echo "设置语言为中文(zh_CN.UTF-8 UTF-8)"
        sed -i '/zh_CN.UTF/s/#//' /etc/locale.gen
locale-gen || /usr/sbin/locale-gen
sed -i '/^export LANG/d' /etc/profile
sed -i '1i\export LANG=zh_CN.UTF-8' /etc/profile
...
;;
	2)
	echo -e "${YELLOW}安装火狐浏览器firefox${RES}"
	pacman -Syu firefox firefox-locale-zh-hans ffmpeg
	if grep -q '^ex.*MOZ_FAKE_NO_SANDBOX=1' /etc/environment; then
		printf "%s\n" "MOZ_FAKE_NO_SANDBOX=1" /etc/environment
	else
		echo 'export MOZ_FAKE_NO_SANDBOX=1' >>/etc/environment
...
uert=$(whoami)
archok=$(cat /etc/os-release | gawk 'NR==1' | gawk -F '"' '{print $2}' | gawk '{print $1}')
echo 当前用户是$uert
sleep 1
echo 开始执行环境检测
sleep 1

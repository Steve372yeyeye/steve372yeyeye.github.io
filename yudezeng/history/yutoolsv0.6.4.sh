...
MAIN(){
     ...
     echo "获取: /sdcard/download/$namel "
      cp /sdcard/download/$namel    ./$namel
      echo -e "正在解压系统包"
      mkdir $sys_name-$AH
	  tar xvf $namel -C $sys_name-$AH 2>/dev/null
      SYS_SET
      FIN
      sleep 1
      ;;
...
echo loading menu
sleep 1
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 15 45 10 "0" "Android-Termux工具(正在开发)" "1" "安装$ARCH_qp容器"  "2" "返回到旧版本页面" "3" "安装恢复包" "4" "导出系统" "5" "安装x64架构的容器(推荐arm64)" "6" "安装X86架构的容器(推荐arm64)" "7" "检查更新" "8" "下载脚本到本地" "10" "退出"  3>&1 1>&2 2>&3)
LODU
}
####
setup(){
...
####
MENU(){
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 15 45 10  "1" "换源" "2" "安装桌面(当前仅支持xfce4)" "3" "新建用户" "4" "containers工具" "5" "下载脚本到本地" "6" "检查更新" "7"  "切换默认语言为中文" "8" "仅配置VNC" "10" "退出"  3>&1 1>&2 2>&3)
LODU_
}
##
LODU_(){

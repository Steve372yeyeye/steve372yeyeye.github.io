...
apt update
echo 检查基础依赖
apt install curl git neofetch wget -y
fi
clear
upytu
}
...
cp -r tmp/pacman.d $sys_name-$AH/etc/pacman.d
rm -rf tmp pacman*
sleep 2
unset LD_PRELOAD 
...
;;
2)
apui=$(whiptail --clear --title "install app" --menu "请选择需要安装的app" 15 45 7 "chromium" "Google Chrome" "" "FireFox 火狐浏览器" "firefox-esr" "FireFox-ESR " "10" "退出"  3>&1 1>&2 2>&3)
case $apui in
chromium)
apt install -y libva2
apt install chromium-browser -y
apt install chromium-browser-l10n -y
apt install chromium-codes-* -y
;;
*)
echo 正在开发
MENU
...
newm=yutoolsv$version.sh https://gitee.com/yudezeng/yutools/raw/master/$newm
wget -o l.log -O $newm 
...
#cp yutools/$newm ./yutools.sh
wget -o l.log -O $newm  https://gitee.com/yudezeng/yutools/raw/master/$newm
chmod 777  -v yutools.sh
rm -rf yutools proot_proc
      if [ $PATHG = Android ]
      then

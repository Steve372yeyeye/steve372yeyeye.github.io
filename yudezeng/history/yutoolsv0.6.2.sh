...
echo loading menu
sleep 2
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 15 45 7  "0" "Android-Termux工具" "1" "安装$ARCH_qp容器"  "2" "返回到旧版本页面" "3" "安装恢复包" "4" "导出系统" "5" "安装x64架构的容器(推荐arm64)" "6" "安装X86架构的容器(推荐arm64)" "7" "检查更新" "8" "下载脚本到本地" "10" "退出"  3>&1 1>&2 2>&3)
exitstatus=$?
...
####
MENU(){
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 15 45 7  "1" "换源" "2" "安装桌面(当前仅支持xfce4)" "3" "新建用户" "4" "containers工具" "5" "下载脚本到本地" "6" "检查更新" "7"  "切换默认语言为中文" "8" "仅配置VNC" "10" "退出"  3>&1 1>&2 2>&3)
exitstatus=$?
if [ $exitstatus = 0 ]; then
    LODU_
else
...
##
SYS_SELECT_(){
opti=$(whiptail --clear --title "选择系统" --menu "选择你要安装的系统" 15 45 7  "1" "debian" "2" "ubuntu" "3" "kali" "4" "centos" "5" "arch" "6" "fedora"  "7" "从本地安装" "0" "退出"  3>&1 1>&2 2>&3)
case $opti in
...
2) input=$(whiptail --clear --title "选择debian的哪个系统？" --menu "请选择" 15 45 7 "1" "ubuntu 18.04 (bionic)" "2" "ubuntu 20.04 (focal)" "3" "ubuntu 22.04 (impish)" "0" "退出"  3>&1 1>&2 2>&3 )                     
	name=ubuntu
	mkdir ubuntu
	case $input in
	1) echo "即将下载安装ubuntu(bionic)"

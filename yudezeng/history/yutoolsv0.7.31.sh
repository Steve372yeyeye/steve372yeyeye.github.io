...
readmek(){
    ...
    echo "yutools v0.7.3 "
    echo 修复了部分已知bug
    echo 添加了使用debootstrap来构建系统
    echo "如果你使用debootstrap来构建系统,debian某些错误能得到避免"
    sleep 2
    echo 即将加载功能...
    sleep 0.6
}
...
apui=$(whiptail --clear --title "proot 容器" --menu "建议优先选择第一项，跨架构容器可能会导致性能损失" 18 50 7  "1" "安装$ARCH_qp Linux 系统 (proot容器)" "01" "使用debootstrap来手动构建系统(可能会出现bug)" "02" "yti-arch(Arch Linux ARM)(arm64)" "03" "ubuntu 22.04 server" "04" "ubuntu 22.10 dev(当前本工具仅支持arm64)" "2" "安装恢复包" "3" "导出系统" "4" "arm64设备安装Linux x86_64系统(proot+qemu)" "5" "arm64设备安装Linux x86系统(proot+qemu)" "10" "退出"  3>&1 1>&2 2>&3)
case $apui in
1)
     ARCH=$(ARCH_)
     AH=$(ARCH_)
...
ldp
sleep 0.5
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 18 50 10 "0" "Android-Termux工具(内置修复termux openssl)" "1" "proot 容器(running linux in termux)" "2" "返回到旧版本页面" "3" "xbtools(雪碧提供支持)" "4" "检查更新" "5" "下载脚本到本地" "00" "退出"  3>&1 1>&2 2>&3)
LODU
}
####
setup(){
...
setdebian(){
    ...
    *) echo 
    ...
    aria2c -s 32 -x 16 -o debs.tar $down_url
    echo 开始解压缩文件...
    mkdir tmp

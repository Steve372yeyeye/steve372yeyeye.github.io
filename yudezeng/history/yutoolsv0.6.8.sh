...
echo version v0.6.7
echo 修复了已知bug
echo 增加了修复termux openssl 3.0报错
echo "更新了对ubuntu/debian (同架构容器) "
echo 自动配置的新支持
echo 自动配置已经能够自动设置中文
echo
echo
...
amd64 | i*86 )
apq=x86
wget -o l.log -O pacman-x86.tar.xz https://pan.xb6868.com/api/v3/file/source/138/pacman-x86.tar.xz?sign=BFiwpfvPks-vT9r8jhrAQlAI7YjkWXaEm6e_JlQD_Ww%3D%3A0
;;
esac
sleep 1
echo 复制文件
...
ldp
sleep 2
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 18 48 8 "0" "Android-Termux工具(内置修复termux openssl)" "1" "安装$ARCH_qp容器" "02" "yti-arch(arm64)" "2" "返回到旧版本页面" "00" "xbtools(雪碧提供支持)" "3" "安装恢复包" "4" "导出系统" "5" "安装x64架构的容器(推荐arm64)" "6" "安装X86架构的容器(推荐arm64)" "7" "检查更新" "8" "下载脚本到本地" "10" "退出"  3>&1 1>&2 2>&3)
LODU
}
####
setup(){
...
#ubuntu/debian TUI
MENU(){
opt=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 20 50 10 "00" "yti adb tools(beta)" "0" "xbtool(qbox工具箱)" "1" "换源" "2" "安装桌面(当前仅支持xfce4)" "3" "新建用户" "4" "containers工具" "5" "下载脚本到本地" "6" "检查更新" "7"  "切换默认语言为中文" "8" "仅配置VNC"  "10" "退出"  3>&1 1>&2 2>&3)
LODU_
}
LODU_(){
    case $opt in
...
echo 当前用户是$uert
if [ `ls /usr/bin/pacman 2>/dev/null ` = "/usr/bin/pacman" ]
then
echo 检测到你可能在使用Arch linux
echo 正在加载此环境适用的脚本
date_t=`date +"%D"`
if ! grep -q $date_t ".date_tmp.log" 2>/dev/null; then
      pacman -Syu libnewt

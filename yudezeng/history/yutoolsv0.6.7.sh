...
source /etc/profile
if (whiptail --title "是否使用yti tools" --yes-button "是的" --no-button "不用"  --yesno "即将安装本作者制作的arch-arm64" 10 60) then   
...
ts(){
    pot=$(whiptail --clear --title "主菜单" --menu "请选择你要执行的功能" 15 45 7  "1" "换源" "2" "修复termux openSSL 3.0报错" "10" "退出"  3>&1 1>&2 2>&3)
opk
}    
opk(){
    case $pot in
...
sleep 1
echo 正在重新执行脚本，请稍等...
if [ `ls /usr/bin/yutoolsw` = "/usr/bin/yutoolsw" ]
then
sudo yutoolsw
else
